...
class SourceWatchRuntime:
    ...
    def __init__(
        ...
        *,
        ...
        # 3. Lexical snapshot root checks
        ...
        # 4. Containment check: snapshot_root cannot be equal to or inside source parent
        ...
        # 5. Initialization without filesystem I/O
    ...
    def _on_adapter_event(
        ...
        with self._lifecycle_lock:
            ...
            if self._async_error is not None:
                return
            try:
                self._coordinator.notify(kind, src_path, destination_path=dest_path)
            except BaseException as exc:
    ...
    def _check_liveness_locked(self) -> None:
        ...
        emitters = getattr(self._observer, "emitters", None)
        if emitters:
            for emitter in emitters:
                if not emitter.is_alive():
                    self._async_error = SourceWatchRuntimeError(
                        SourceWatchRuntimeReason.OBSERVER_STOPPED_UNEXPECTEDLY,
                        "Observer emitter thread stopped unexpectedly",
                    )
                    self._admission_open = False
                    return
        ...
        try:
            ...
            try:
                ...
                if isinstance(observer, threading.Thread):
                    self._owned_threads.add(observer)
            except BaseException as exc:
            ...
            # 3. Start observer and capture emitters
            try:
                observer.start()
                emitters = getattr(observer, "emitters", None)
                if emitters:
                    for em in emitters:
                        if isinstance(em, threading.Thread):
                            self._owned_threads.add(em)
            except BaseException as exc:
                # Capture any emitters that were started before failure
                emitters = getattr(observer, "emitters", None)
                if emitters:
                    for em in emitters:
                        if isinstance(em, threading.Thread):
            ...
            # 4. Enqueue initial logical MODIFIED notice to coordinator
            with self._lifecycle_lock:
                if self._async_error is not None:
            ...
            threads_to_join = list(self._owned_threads)
            if self._observer is not None and self._observer not in threads_to_join:
                threads_to_join.append(self._observer)
            if self._observer is not None and hasattr(self._observer, "emitters"):
                for em in self._observer.emitters:
                    if em not in threads_to_join:
                        threads_to_join.append(em)
            ...
            if start_error is not None:
                all_errors.append(start_error)
            if run_error is not None:
                all_errors.append(run_error)
            with self._lifecycle_lock:
                if self._async_error is not None:
                    all_errors.append(self._async_error)
            all_errors.extend(teardown_errors)
            ...
            # Raise single error or ExceptionGroup/BaseExceptionGroup if any
            if all_errors:
                if len(all_errors) == 1:
                    raise all_errors[0]
                ...
                std_excs = [e for e in all_errors if isinstance(e, Exception)]
