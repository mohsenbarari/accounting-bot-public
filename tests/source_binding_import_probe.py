"""Guard exact binding-module execution; dependency/bootstrap reads are outside it.

The loader fetches code before guarding execution. No path is allowlisted while
the target executes. A canary injects a write into the real target module scope.
The same guard is used after fixture construction for evaluation purity tests.
"""
...
TARGET = "accounting_contracts.source_binding"
PUBLIC_NAMES = (
...
def forbidden(*args: Any, **kwargs: Any) -> Any:
    raise ForbiddenSideEffect("Source binding side-effect guard")
...
class GuardedLoader(importlib.abc.Loader):
    ...
    def exec_module(self, module: ModuleType) -> None:
        # Bootstrap dependencies outside the target execution guard.
...
def main() -> int:
    ...
    assert module.SOURCE_BINDING_VERSION == "source-binding.v1"
